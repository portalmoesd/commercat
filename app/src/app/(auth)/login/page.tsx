...
export default function LoginPage() {
  ...
  async function handleGoogleLogin() {
    ...
    const { error: authError } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${process.env.NEXT_PUBLIC_APP_URL || window.location.origin}/auth/callback?redirect=${encodeURIComponent(redirectTo)}` },
    });
    ...
    <div className="min-h-screen flex items-center justify-center bg-cream px-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-center text-xs text-gray-dark mt-6">
          ...
          <Link href="/signup" className="text-charcoal font-medium underline">
            Sign up
          </Link>
        </p>
      </div>
